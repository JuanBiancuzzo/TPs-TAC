%% Identificacion
%  Primero correr el simulink para tener los valores de out
angulo = out.angulo;
theta = out.theta;
tiempo = out.tout;
dt = 0.02;

% Definimos el modelo de la planta
% theta/u = k / ( (s + p1) * (s + p2) )
% theta/u|discreto = c0 / ( (z + c1) * (z + c2) )
% yn = D0 u_{n-2} + D1 y_{n-1} + D2 y_{n-2}
% y => theta
% u => angulo

largo = size(angulo);
largo = largo(1);

X = [ angulo(3:largo) theta(2:largo-1) theta(1:largo-2) ]';
y = theta(3:largo);

D = inv(X * X') * X * y;

% Calculamos valores de transferencia a partir los D
C0 = -D(1) / D(3);
C2 = (-D(2) + sqrt(D(2) * D(2) + 4 * D(3))) / (-2 * D(3));
C1 = ( D(2) / D(3) ) - C2;

C = [ C0 C1 C2 ];
%% Validacion
% Simulamos curva con nuestros parametros
z = tf('z', dt);

T = C0 / ((z + C1) * (z + C2));
y_simulada = lsim(T, angulo, tiempo);

figure
hold on 
grid on

plot(tiempo, y_simulado, 'y-')
plot(tiempo, theta, 'r-')

%% Simulacion en el tiempo con ecuacion en diferencias

y_simulado = zeros(largo, 1);
y_simulado(1) = theta(1);
y_simulado(2) = theta(2);

for i = 3:largo
    y_simulado(i) = D(1) * angulo(i) + D(2) * y_simulado(i - 1) + D(3) * y_simulado(i - 2);
end 

figure
hold on 
grid on

plot(tiempo, y_simulado, 'y-')
plot(tiempo, theta, 'r-')

%% Segunda validacion
% Otras muestras y los mismos D

angulo_nuevo = out.angulo;
theta_nuevo = out.theta;
tiempo_nuevo = out.tout;

largo = size(angulo_nuevo);
largo = largo(1);

y_simulado = zeros(largo, 1);
y_simulado(1) = theta_nuevo(1);
y_simulado(2) = theta_nuevo(2);

for i = 3:largo
    y_simulado(i) = D(1) * angulo_nuevo(i) + D(2) * y_simulado(i - 1) + D(3) * y_simulado(i - 2);
end 

figure
hold on 
grid on

plot(tiempo_nuevo, y_simulado, 'y-')
plot(tiempo_nuevo, theta_nuevo, 'r-')
