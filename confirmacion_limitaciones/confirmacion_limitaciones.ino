...
void loop() {
  ...
  // Enviar datos
  enviar_datos(angulo_control, theta_complementario);
...
void enviar_datos(float control, float theta_complementario){  
  // Enviar header
  Serial.write("abcd");

  const int cant_mediciones = 2;
  float mediciones[cant_mediciones] = { control, theta_complementario };
