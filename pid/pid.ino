...
const float SESGO_ANGULO = -1.17f;
const float SESGO_ANGULAR = 0.0f;
...
typedef struct {
  float err_actual;
  float acumulada_integral;
  float acumulada_derivativo;
  float posicion_ref;
} info_control_t;
...
float calcular_angulo_complementario(float theta_anterior, sensors_vec_t* velocidad_angular, sensors_vec_t* aceleracion) {
  ...
  float theta_giroscopio = theta_anterior + RADIANES_2_GRADOS * (velocidad_angular->x - SESGO_ANGULAR) * ((float)(periodo_millis) / 1000.0f);
...
float avanzar_control(float posicion_medida, info_control_t* control_prev) {
  ...
  control_prev->acumulada_integral = factor_integral;
  control_prev->acumulada_derivativo = factor_derivativo;
